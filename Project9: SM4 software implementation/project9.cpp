...
int main() {
    ...
    uint32_t plaintext1[8] = { 0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210 ,0x01234567, 0x89abcdef, 0xfedcba98, 0x76543210 };
    ...
    uint32_t decrypted1[8];
    uint8_t plaintext5[64] = { 0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,
0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1A,0x1B,0x1C,0x1D,0x1E,0x1F,
0x20,0x21,0x22,0x23,0x24,0x25,0x26,0x27,0x28,0x29,0x2A,0x2B,0x2C,0x2D,0x2E,0x2F,
0x30,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x3A,0x3B,0x3C,0x3D,0x3E,0x3F,
    };
    int iterations = 175872; // 用于性能测试的迭代次数
    ...
    if (memcmp(plaintext1, decrypted1, sizeof(plaintext1)) != 0) {
        printf("Custom SM4 implementation correctness test failed\n");
        return 1;
    }

    key_expansion(key, rk);
    sm4_encrypt1(plaintext, rk, ciphertext);
    sm4_decrypt1(ciphertext, rk, decrypted);
    ...
    printf("Custom SM4 implementation time: %f seconds\n", custom_time);
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        sm4_decrypt(ciphertext, rk, decrypted);
    }
    custom_end = clock();
    custom_time = ((double)(custom_end - custom_start)) / CLOCKS_PER_SEC;
    printf("Custom SM4 implementation time: %f seconds\n", custom_time);
    if (memcmp(ciphertext1, ciphertext, sizeof(ciphertext)) != 0) {
        printf("Custom SM4 implementation correctness test failed\n");
        return 1;
    }
    ...
    key_expansion(key, rk);
    for (int i = 0; i < iterations; ++i) {
        sm4_top2(plaintext5, 64, key, ciphertext5);
    }
    custom_end = clock();
    custom_time = ((double)(custom_end - custom_start)) / (CLOCKS_PER_SEC*8);
    printf("sm4 top 2 Custom SM4 implementation time: %f seconds\n", custom_time);
    custom_start = clock();
    key_expansion(key, rk);
    for (int i = 0; i < iterations; ++i) {
        sm4_top(plaintext5, 64, key, ciphertext5);
    }
    custom_end = clock();
    custom_time = ((double)(custom_end - custom_start)) / (CLOCKS_PER_SEC * 8);
    printf("sm4 top 1 Custom SM4 implementation time: %f seconds\n", custom_time);
    if (memcmp(ciphertext1, ciphertext, sizeof(ciphertext)) != 0) {
        printf("Custom SM4 implementation correctness test failed\n");
        return 1;
    }
