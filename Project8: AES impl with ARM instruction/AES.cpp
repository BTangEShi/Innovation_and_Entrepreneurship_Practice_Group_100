...
int main()
{
    int8_t plaintext[] = { 0x32, 0x88, 0x31, 0xe0, 0x43, 0x5a, 0x31, 0x37, 0xf6, 0x30, 0x98, 0x07, 0xa8, 0x8d, 0xa2, 0x34 };
    int8_t key[] = { 0x2b, 0x28, 0xab, 0x09, 0x7e, 0xae, 0xf7, 0xcf, 0x15, 0xd2, 0x15, 0x4f, 0x16, 0xa6, 0x88, 0x3c };
    int8_t computed_cipher[16];
    aes128_load_key(key);
    aes128_enc(plaintext, computed_cipher);
    std::cout << "Key elements (hex): ";
    for (int i = 0; i < 16; ++i) {
        std::cout << std::hex << std::setw(2) << std::setfill('0') << static_cast<uint32_t>(computed_cipher[i] & 0xFF) << " ";
    }
    std::cout << std::endl;
